...
export default defineConfig({
  ...
  integrations: [
    sitemap({
      lastmod: new Date(),
      changefreq: 'weekly',
      filter: (page) => !page.includes('/brand-preview') && !page.includes('/motion-lab') && !page.includes('/search-results') && !page.includes('/privacy') && !page.includes('/terms') && !page.includes('/accessibility'),
      serialize(item) {
        const url = item.url;
